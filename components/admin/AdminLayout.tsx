...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const current = router.pathname === '/admin' ? 'dashboard' : router.pathname.split('/')[2] || 'dashboard'
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <nav className="fixed top-0 left-0 w-full bg-white border-b border-stone-200 px-8 py-3 flex items-center justify-between z-50">
        ...
        <Link href="/account" className="text-stone-800 hover:underline font-medium">My Account</Link>
      </nav>
      <div>
        {/* Sidebar */}
            ...
            <nav className="flex-1 px-6 py-4 space-y-2">
              {SECTIONS.map((s) => (
                <Link key={s.key} href={s.href} legacyBehavior>
                  <a className={`block w-full text-left px-4 py-2 rounded ${current === s.key ? 'bg-blue-100 text-blue-700 font-semibold' : 'hover:bg-gray-100 text-gray-700'}`}>{s.label}</a>
                </Link>
              ))}
            </nav>
          </div>
        ...
        {/* Main content */}
        <main className="flex-1 flex flex-col pl-64 pt-[56px]">
          {children}
        </main>
      </div>
    </div>
